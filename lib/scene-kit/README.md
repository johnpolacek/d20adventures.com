...
| `cameras` | named shots `{ position, target, label, fov? }`. **The first is the establishing shot.** |
...
## Exemplars
`scripts/scene-pipeline/exemplars/<encounter-id>/` holds the brief, reference image and standalone scene each set was extracted from. `lib/scene-sets/realm-of-myr/kordavos-outer-gate.ts` is the reference implementation of a set.
