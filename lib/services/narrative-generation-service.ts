...
import { limitToTwoSentences, fixMalformedQuotes } from "@/lib/utils/narrative-utils";
...
export async function formatNarrativeAction({
  ...
  if (characterInfo?.specialAbilities && characterInfo.specialAbilities.length) infoLines.push(`Abilities: ${characterInfo.specialAbilities.slice(0, 3).join(', ')}`);
  if (characterInfo?.skills && characterInfo.skills.length) infoLines.push(`Skills: ${characterInfo.skills.slice(0, 3).join(', ')}`);
  if (characterInfo?.equipment && characterInfo.equipment.length) infoLines.push(`Equipment: ${characterInfo.equipment.slice(0, 2).map(e => e.name).join(', ')}`);
