...
Respond as JSON:
{
  ...
  if (rollRequirement && rollRequirement.rollType && rollRequirement.difficulty) {
    ...
    updatedCharacters = updatedCharacters.map((c) => {
      ...
      const updated = { ...c };
      if (effect) {
        updated.healthPercent = effect.healthPercentDelta !== undefined ? Math.max(0, (c.healthPercent ?? 100) + effect.healthPercentDelta) : c.healthPercent;
    ...
    console.log("[NPC TURN] Roll required. Appending shortcode and outcome narrative only:");
    console.log("[NPC TURN] Shortcode:", shortcode);
    console.log("[NPC TURN] Outcome narrative:", outcomeResult.narrative);
