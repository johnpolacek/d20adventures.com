...
export function mapConvexTurnToTurn(raw: unknown): import("@/types/adventure").Turn | null {
  if (!raw || typeof raw !== "object" || !("encounterId" in raw)) return null;
  const t = raw as { _id: string; encounterId: string; title?: string; subtitle?: string; narrative: string; characters: import("@/types/adventure").TurnCharacter[]; adventureId: string; isFinalEncounter?: boolean };
  return {
    id: t._id,
