...
export function getPrimaryAttributeForRoll(rollType: string): keyof Attributes | null {
  ...
  // Dexterity-based
  if (rollTypeLower.includes("stealth") || rollTypeLower.includes("acrobatics") || rollTypeLower.includes("sleight of hand") || rollTypeLower.includes("dexterity")) {
    return "dexterity"
  }

  // Constitution-based
  if (rollTypeLower.includes("constitution") || rollTypeLower.includes("endurance")) {
    return "constitution"
  }

  // Intelligence-based
  if (
    rollTypeLower.includes("investigation") ||
  ...
  // Wisdom-based
  if (
    rollTypeLower.includes("perception") ||
  ...
  // Charisma-based
  if (
    rollTypeLower.includes("persuasion") ||
...
export function calculateAttributeModifier(character: unknown, rollType: string): number {
  ...
  if (!character || typeof character !== "object" || !("attributes" in character) || !character.attributes || typeof character.attributes !== "object") {
    return 0
  }
