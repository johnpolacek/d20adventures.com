...
export function analyzePlayerInput(input: string, characterName: string) {
  ...
  const hasSingleQuoteDialogue = /'[A-Z][^']*[.!?,]'\s*\w+\s*(said|says|asks|replies|shouts|whispers|calls|thinks|shouted|yelled|screamed|cried)/i.test(input) ||
  ...
  const hasDialogueTags = input.includes(' says') || input.includes(' asks') || input.includes(' replies') ||
    input.includes(' states') || input.includes(' whispers') || input.includes(' shouts') ||
    input.includes(' calls') || input.includes(' thinks') || input.includes('" she ') || 
    input.includes('" he ') || input.includes('" they ');
  ...
  const hasPastTense = /\b(approached|walked|said|asked|looked|moved|took|drew|attacked|cast|ran|went|came|did|was|were)\b/i.test(input) &&
                       !/\b(states|thinks|weaves)\b/i.test(input);
  ...
  return {
    ...
    preserveOriginal: (hasGoodStructure || hasDetailedAction || (hasDialogue && hasDialogueTags && isThirdPerson && !hasPastTense) || (hasDialogue && hasDialogueTags && !isMinimal)) && !(isFirstPerson && !hasDialogue),
