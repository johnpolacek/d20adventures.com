...
export const getCurrentAdventureWithNpcProcessing = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (adventure.currentTurnId) {
      currentTurn = await ctx.db.get(adventure.currentTurnId as Id<"turns">);
      console.log("[getCurrentAdventureWithNpcProcessing] loaded currentTurn:", JSON.stringify(currentTurn, null, 2));
    } else {
...
// Moved setPlayerRollResult mutation to convex/rolls.ts to keep this file under the line limit.
// import { setPlayerRollResult } from "./rolls";
