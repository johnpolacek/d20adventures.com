...
import { Check, Download, FileText, MessageSquare, RefreshCcw, Save, Upload, Wand2 } from "lucide-react"
...
export function AdminWikiAdventureEditor({ initialState }: { initialState: EditorState }) {
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] bg-[#10130f] text-stone-100">
      <header className="border-b border-lime-900/60 bg-[#151912] px-5 py-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="text-sm text-stone-400">{state.files.length} files · {Object.keys(state.encounters).length} encounters · changes apply to canonical wiki source</p>
          </div>
          <div className="flex flex-wrap gap-2">
      ...
      <main className="grid min-h-[calc(100vh-172px)] grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)_420px]">
        <aside className="border-b border-lime-900/60 bg-[#12170f] xl:border-r xl:border-b-0">
          <div className="border-b border-lime-900/50 px-4 py-3">
            <h2 className="font-mono text-xs font-bold uppercase text-lime-300">Wiki Files</h2>
          </div>
          <div className="max-h-[360px] overflow-auto p-2 xl:max-h-[calc(100vh-230px)]">
            {state.files.map((file) => (
              <button
                key={file.path}
                type="button"
                onClick={() => setSelectedPath(file.path)}
                className={`mb-1 grid w-full grid-cols-[20px_minmax(0,1fr)] items-center gap-2 rounded-md border px-2 py-2 text-left ${file.path === selectedPath ? "border-amber-500/70 bg-amber-950/25" : "border-transparent hover:border-lime-900/60 hover:bg-lime-950/20"}`}
              >
        ...
        <section className="min-w-0 border-b border-lime-900/60 bg-[#0f130d] p-4 xl:border-r xl:border-b-0">
          <KeyFieldEditor file={selectedFile} manifest={state.manifest} encounters={state.encounters} disabled={busy} onSave={saveFile} />
        </section>

        <aside className="bg-[#141711] p-4">
          <div className="flex items-center gap-2 text-amber-200">
            <MessageSquare className="size-4" />
            <h2 className="font-mono text-xs font-bold uppercase">Chat With Wiki</h2>
          </div>
          <div className="mt-3 h-[calc(100vh-320px)] min-h-[340px] space-y-3 overflow-auto rounded-md border border-lime-900/60 bg-black/20 p-3">
