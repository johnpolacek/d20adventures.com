...
import { hasBooleanProp, hasNumberProp, isRollRequired } from "@/lib/utils"
...
export default function TurnNarrativeReply({ character, submitReply }: TurnNarrativeReplyProps) {
  ...
  const hasReplied = hasBooleanProp(characterState, "hasReplied") ? characterState.hasReplied : undefined
  const rollRequired = characterState && typeof characterState === "object" && "rollRequired" in characterState ? (characterState as { rollRequired?: unknown }).rollRequired : undefined
  ...
  const handleRegularReply = async () => {
    ...
    try {
      ...
      console.log("[handleRegularReply] aiResult:", JSON.stringify(aiResult, null, 2))
      if (typeof aiResult !== "string") {
        console.error("[handleRegularReply] aiResult is not a string:", aiResult)
        setError("Failed to format reply. Please try again.")
        setLoading(false)
        return
      }
      if (typeof currentTurn.id !== "string") {
        console.error("[handleRegularReply] currentTurn.id is not a string:", currentTurn.id)
      ...
      console.error("[handleRegularReply] Error:", err)
  ...
  const handleRollResult = async (result: number) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to process roll result. Please try again.")
    } finally {
      setInput("")
      setLoading(false)
    }
  ...
  return (
    <form onSubmit={handleReply} className="flex flex-col gap-4 min-h-[100px]">
      ...
      {isRollRequired(rollRequired) && rollResult == null && rollRequired.rollType != null && rollRequired.rollType !== "null" && typeof rollRequired.difficulty === "number" && (
        <CharacterDiceRoll
          character={characterState as TurnCharacter}
          rollType={rollRequired.rollType}
          difficulty={rollRequired.difficulty}
