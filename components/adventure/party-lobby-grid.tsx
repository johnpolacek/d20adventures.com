...
export function PartyLobbyGrid({
  adventure,
  adventurePlan,
  userId,
  onCharacterClick,
    ...
    if (i === 1 && userChar) {
      ...
      slots.push(
        <PartySlot
          ...
          onClick={() =>
            onCharacterClick ? onCharacterClick(userChar!) : undefined
          }
        />
      );
    } else if (charIdx < otherChars.length) {
      const character = otherChars[charIdx];
      const playerName = character.userId
        ? playerNames[character.userId]
        : undefined;
      slots.push(
        <PartySlot
          key={character.id}
      ...
      slots.push(
        <PartySlot
          ...
          onJoinClick={
            joinableChar && onJoinClick
              ? () => onJoinClick(joinableChar.id)
              : undefined
          }
          isJoining={isJoining}
        />
  ...
  return (
    <div
      className={
        "flex flex-wrap gap-6 justify-center items-center w-full max-w-6xl mx-auto mb-8"
      }
    >
