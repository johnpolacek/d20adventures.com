...
export default function DiceRollResult({ result, animate }: { result: number; animate?: boolean }) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center">
      <div className="w-20 h-20 flex items-center justify-center text-4xl text-white bg-black/70 border-2 border-blue-300/30 ring-8 ring-white/5 rounded-full">
        {animatedResult ? <span className="font-mono">{animatedResult}</span> : <span className="font-display">~</span>}
      </div>
      <div className="-mt-2 flex justify-center w-48">
        {result && (
          <div
            style={{ boxShadow: "inset 0 -4px 16px 0 rgba(0,0,0,.5), 0 0 1px 2px #000" }}
            className="flex gap-1.5 items-center justify-center w-32 bg-blue-500 border border-white/30 font-display rounded-full pr-4 text-lg bg-[url('/images/app/backgrounds/buried.png')] brightness-110 contrast-150 saturate-150 shadow-inner"
          >
