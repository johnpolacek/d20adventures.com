...
export default function AdventureLobby({
  ...
  const [joiningCharacterId, setJoiningCharacterId] = useState<string | null>(
    null
  );
  const [modalCharacter, setModalCharacter] = useState<PC | PCTemplate | null>(
    null
  );
  ...
  const availableCharacters =
    adventurePlan?.premadePlayerCharacters?.filter(
      (pc) => !party?.some((partyMember) => partyMember.id === pc.id)
    ) || [];
  ...
  const inviteLink =
    typeof window !== "undefined"
      ? `${window.location.origin}/settings/${params.settingId}/${params.adventurePlanId}/${adventure.id}`
      : "";
      ...
      <div className="w-full flex flex-col items-center justify-center max-w-6xl fade-in relative z-10 pb-16 -mt-16">
        ...
        {isSignedIn && !userCharacter && (
          <div className="w-full flex flex-col items-center justify-center mb-8">
            {isLoadingUserChars ? (
              <div className="text-center text-white/80 py-8 opacity-0">
                Loading your characters...
              </div>
            ) : (
              <>
                {joinError && !/redirect/i.test(joinError) && (
                  <div className="text-center text-red-400 text-sm mb-4 bg-red-900/20 px-4 py-2 rounded">
                    {joinError}
                  </div>
                )}
                {userCharacters.length > 0 && (
                  <>
                    <h2 className="text-2xl font-display text-amber-400 mb-6">
                        ...
                        return (
                          ...
                          >
                            <CharacterSelectCard
                              ...
                              character={char}
                              buttonLabel={
                                isJoining && joiningCharacterId === charJoinId
                                  ? "Joining..."
                                  : "Join"
                              }
                              disabled={isJoining}
                              onButtonClick={() =>
                                handleJoinAdventure(charJoinId)
                              }
                            />
                          </div>
                        );
                ...
                <Link
                  href={`/player/${user?.username}/characters/new?redirectToAdventure=${encodeURIComponent(window.location.pathname)}`}
                >
        ...
        {isSignedIn && userCharacter && hasEmptySlots && (
