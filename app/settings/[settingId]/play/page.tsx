...
export default async function SettingAdventures(props: { params: Promise<{ settingId: string }> }) {
  ...
  const adventureLinks = () => {
    return (
      <div className="flex justify-center gap-6 relative z-10 px-4 pt-8">
        {adventures.map((adventure) => (
          <Link key={adventure.id} href={`/settings/${settingId}/${adventure.id}`} className="block">
            <Card className="w-80 bg-black/80 border-white/20 scale-95 hover:scale-100 hover:bg-black/90 ring-4 ring-black hover:ring-8 hover:ring-primary-500 transition-colors cursor-pointer transition-all duration-500 ease-in-out p-0 overflow-hidden">
              <div className="pb-2 relative aspect-[2.25] w-full">
                ...
                <Image src={`${IMAGE_HOST}/${adventure.image}`} alt={adventure.title} fill className="object-cover" />
              </div>
              <CardContent>
                <div className="relative z-10">
                  <div className="text-gray-300 text-base line-clamp-3 -mt-2">{adventure.teaser || adventure.overview}</div>
  ...
  return (
    <div className="flex min-h-screen flex-col relative">
      <div className="fade-in delay-[2s] relative z-10">
        <ImageHeader imageUrl={`${IMAGE_HOST}/${setting.image}`} title={setting.name} imageAlt={setting.name}>
          {adventureLinks()}
        </ImageHeader>
        <div className="max-w-3xl mx-auto -mt-24 relative z-10 whitespace-pre-line">
