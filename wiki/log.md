...
- **Drift investigated → prod S3 is canonical.** A content comparison of the 40 drifted files showed March of Davos was only rawly/partially migrated into the repo; the finished content lives in prod S3 (refined via the wiki authoring tools post-migration). Prod's `adventure.md` carries `Aftermath`/`Conclusion`/`Sequel Hooks` sections the repo lacks; prod's climax encounter (`final-confrontation`) is plot-coherent (named antagonist Joran, the Key of Ilmarin, branching resolution) where the repo is a generic draft; prod NPC sheets are ~2× fuller. Not a clean superset — ~17 files are longer in the repo (likely verbose raw-migration prose prod later tightened). **Recommended remediation:** reconcile prod S3 → repo (mirror the prod source into git), spot-check the repo-longer files, then add `availableCharacterOptions` and deploy. The other three adventures need no action.
