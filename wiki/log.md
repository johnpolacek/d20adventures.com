...
- **Decision needed:** which March of Davos wiki source is canonical — the repo (overwrite/clear prod S3 so it uses repo-bundled source) or prod S3 (reconcile the 40 drifted files back into the repo, then add `availableCharacterOptions`)? Resolving it requires prod S3 writes; not done unilaterally. The other three adventures need no action.
