...
import { applyAuthoringChangeSet, ChangeSetError, compileAdventureSourceTree, createSourceFile, createSourceTree, InMemoryWikiAdventureSourceService, S3WikiAdventureSourceService } from "@/lib/wiki-adventures"
...
assert.equal(invalidCompile.validationReport.findings.some((finding) => finding.code === "character.sheet.invalid"), true)
...
await s3Service.writeApprovedChangeSet({
  ...
  source: "human",
  target: { settingId: "myr", planId: "the-old-road", draftId: "active" },
  changes: [{ op: "rename", fromPath: s3File.path, toPath: "content/settings/myr/adventures/the-old-road/adventure-renamed.md", beforeHash: createSourceFile(s3File.path, "new-from-s3-service").hash }],
  affectedEntities: [{ type: "adventure", id: "the-old-road" }],
  risks: [],
})
